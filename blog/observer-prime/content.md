...
- **The creator is the editor.** AI proposes; people decide.
...
- **The orchestrator** is the heart of the system. All workflow rules live in one module: which steps are unlocked, how versions are tracked, what gets cleared when an earlier choice changes, and exactly what context each AI call receives.
...
## The most important decision: we threw away V1
...
| **V1.1** | Replaced by a *sequential* wizard: Topic → Title → Story → Hook → Key Points → Final Prompt. |
...
| **V3** | YouTube publishing: upload preparation, description and chapters, thumbnail concepts, AI-disclosure guidance, scheduling | Google Cloud project and YouTube Data API access |
| **V4** | Analytics feedback loop: import click-through rate, retention and views, link them to each video's title, hook and structure, and feed "what worked for this channel" back into the default prompts | V3 being in place |
...
The AI does the heavy lifting; accountability stays with people. And when the elegant version doesn't serve the user, have the discipline to replace it.
