...
- **Android:** the core screens are working: welcome, home, chat, history, bookmarks and profile. Settings is still a placeholder, and there is no automated Android test suite yet. That is the most important gap to close next.

## What's next

Near term, we're closing gaps in shipped features: encrypting on-device history at rest, a real Settings screen, editing bookmark notes, streaming replies, and Android tests.

After that, the product roadmap in priority order:
...
The most useful lesson here isn't about the Gita. It applies to **any domain where being wrong is expensive**: legal, medical, financial, compliance. The pattern is the same:
